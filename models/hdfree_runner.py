...
# Modified from: models/rdt_runner.py
class HDFreeRunner(
    nn.Module,
    ...
    def __init__(self, *, action_dim, pred_horizon, config,
        ...
        super(HDFreeRunner, self).__init__()
        ...
        self.use_da_vis1: bool = True
        self.use_da_vis2: bool = True
        self.use_da_act: bool = True
        self.act_loss_from: str = "mlp"
        self.act_layers: int = 28
        self.act_weights: str = "adapter"  # ia3

        self.cache_perceptual_embedding = None
        self.cache_timesteps = None
        self.cache_timestep_embedding = None
        self.cache_mlp_output = None

    def register_modules_for_target(self):
        # Register Adapter blocks for trainable modules
        if 'adapter' in self.act_weights:
            self.model.register_adapter()

    def unfreeze_vis(self):
        trainable_params = []
        for name, p in self.img_adaptor.named_parameters():
            p.requires_grad = True
            trainable_params.append(p)
        print("[DEBUG] unfreeze_vis:", len(trainable_params))
    ...
    def unfreeze_act(self):
        ...
        for name, p in self.model.named_parameters():
            if p.requires_grad:
                trainable_params.append(p)
        print("[DEBUG] unfreeze_act:", len(trainable_params))
        return trainable_params
    ...
    def compute_loss(self, lang_tokens, lang_attn_mask, img_tokens,
        ...
        ).long() if timesteps is None else timesteps
        self.cache_timesteps = timesteps
        ...
        lang_cond, img_cond, state_action_traj = self.adapt_conditions(
            lang_tokens, img_tokens, state_action_traj)  # lang_cond:(B,16,2048)
        self.cache_perceptual_embedding = img_cond  # For DA, (B,4374,2048)*28

        # Predict the denoised result
        pred = self.model(state_action_traj, ctrl_freqs,
                          timesteps, lang_cond, img_cond,
                          lang_mask=lang_attn_mask)
        self.cache_timestep_embedding = self.model.cache_timestep_embedding  # (B,1,2048)
        self.cache_mlp_output = self.model.cache_mlp_out  # List, (B,67,2048)
        # print('[DEBUG] mlp_len:', len(self.cache_mlp_output))
        # print('[DEBUG] cache_timestep_embedding:', self.cache_timestep_embedding.shape)
        # print('[DEBUG] cache_timesteps:', self.cache_timesteps.shape, self.cache_timesteps)
